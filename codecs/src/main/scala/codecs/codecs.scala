...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromFunction {
      case Json.Num(value) => if (value.isValidInt) Option(value.intValue) else Option.empty
      case _ => Option.empty
    }
  ...
  given Decoder[Boolean] =
    Decoder.fromFunction {
      case Json.Bool(value) => Option(value)
      case _ => Option.empty
    }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(arr) => if (arr.isEmpty) Option(List())
                            else Option(arr.map { a => decoder.decode(a).get })
      case _ => Option.empty
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(fields) => if (fields.isEmpty) Option.empty else decoder.decode(fields.get(name).get)
      case _ => Option.empty
    }
  ...
  given Decoder[Person] =
    Decoder.fromFunction {
      case x => {
        ...
        val name = Decoder.field("name")(using implicitly[Decoder[String]]).decode(x) 
        val age = Decoder.field("age")(using implicitly[Decoder[Int]]).decode(x) 

        if (name.isEmpty || age.isEmpty) {
          Option.empty
        } else {
          Option(Person(name.get, age.get)) 
        }         
      }
    }
...
trait ContactsCodecs:
  ...
  Decoder.fromFunction {
    case x => {
      ...
      val people = Decoder.field("people")(using implicitly[Decoder[List[Person]]]).decode(x)

      if (people.isEmpty) {
        Option.empty
      } else {
        Option(Contacts(people.get))
      }
    }
  }
