...
trait Solver extends GameDef:
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = LazyList.from(b.legalNeighbors).map { n => (n._1, n._2 :: history)}
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) {
      ...
      val newN = newNeighborsOnly(neighborsWithHistory(initial.head._1, initial.head._2), explored)
      val value = initial.lazyAppendedAll(from(initial.drop(1).lazyAppendedAll(newN), explored + initial.head._1 ))
      value
    }
  }
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.map { p => p._2.reverse }.appended(List()).head
